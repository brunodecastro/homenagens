...
import views.html.tipoHomenagem.*;
...
public class TipoHomenagens extends Controller {
    ...
    public static Result GO_HOME = redirect(
        routes.TipoHomenagens.list(0, "name", "asc", "")
    );
    ...
    public static Result list(int page, String sortBy, String order, String filter) {
        return ok(
            list.render(Usuario.consultarPorEmail(request().username()), 
            		TipoHomenagem.page(page, 10, sortBy, order, filter),
                sortBy, order, filter
            )
        );
    }
     ...
     *
     * @param id Id of the tipoHomenagem to edit
     */
    public static Result edit(Long id) {
    	TipoHomenagem tipoHomenagem = TipoHomenagem.find.byId(id);
        Form<TipoHomenagem> tipoHomenagemForm = form(TipoHomenagem.class).fill(
        		tipoHomenagem
        );
        return ok(
            editForm.render(Usuario.consultarPorEmail(request().username()), id, tipoHomenagemForm, TipoHomenagem.getSubs(tipoHomenagem))
        );
    }
    ...
    public static Result create() {
        ...
        return ok(
            createForm.render(Usuario.consultarPorEmail(request().username()), tipoHomenagemForm)
        );
    }
    ...
    public static Result createSub(Long parentId) {
    	...
    	return ok(
    		createSubForm.render(Usuario.consultarPorEmail(request().username()), parentId, tipoHomenagemForm)
    	);
    }
    ...
    public static Result editSub(Long id) {
        ...
        return ok(
            editSubForm.render(Usuario.consultarPorEmail(request().username()), tipoHomenagem.parent.id, id, tipoHomenagemForm)
        );
    }
    ...
    public static Result deleteSub(Long id) {
    	...
    	String deletedName = tipoHomenagem.name;
    	tipoHomenagem.delete();
        flash("success", "O subTipo de Homenagem \"" + deletedName + "\" foi excluído");
